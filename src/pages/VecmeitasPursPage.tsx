...
const VecmeitasPursPage = () => {
  const [isVisible, setIsVisible] = useState(false);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const sectionRef = useRef<HTMLDivElement>(null);
  const { t } = useTranslation('vecmeitas');
  ...
  const vecmeituImages = [
    ...
    'https://wigoyeorqnssgbrgexku.supabase.co/storage/v1/object/public/websiteassets/bildes/50.png',
    'https://wigoyeorqnssgbrgexku.supabase.co/storage/v1/object/public/websiteassets/bildes/72.png'
  ];

  const handleImageNavigation = (direction: 'prev' | 'next') => {
    if (direction === 'prev') {
      setCurrentImageIndex((prev) => (prev + vecmeituImages.length - 1) % vecmeituImages.length);
    } else {
      setCurrentImageIndex((prev) => (prev + 1) % vecmeituImages.length);
    }
  };
  ...
  const vecmeituIncludedItems = [
    'Kopēja draudzeņu sildīšanās ar novēlējumu rituālu;',
    'Vibroakustiskā masāža ar klūgu slotām gaviļniecei;',
    'Relaksējošs pēriens, kura laikā draudzenes palīdz pirtniekam ar sildīšanu, slotu kompresēm;',
    'ATTĪROŠAIS SKRUBIS – attīroša noberšanās ar augu maisījumiem, kas attīra ķermeni un garu;',
    'Galvenais pēriens individuāli gaviļniecei;',
    'BAROJOŠAIS SKRUBIS – (iepriekš vienojoties) kafijas, dzintara, mālu, medus vai cita ķermeņa kopšanas līdzekļa uzklāšana gaviļniecei;'
  ];
        ...
        <section ref={sectionRef} className="py-20 px-6 max-w-7xl mx-auto">
          ...
          <div className={`max-w-3xl mx-auto mb-16 transform transition-all duration-1000 ${
            ...
            <div className="bg-[#132d13] rounded-2xl p-8 border-l-4 border-green-500">
              <h3 className="text-xl font-bold text-white mb-4 text-center">{t('poem.title')}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-20">
            ...
            <div className={`transform transition-all duration-1000 ${
              ...
              <div className="relative rounded-2xl overflow-hidden group">
                {/* Images */}
                {vecmeituImages.map((image, index) => (
                  <div
                    key={index}
...
export default VecmeitasPursPage;